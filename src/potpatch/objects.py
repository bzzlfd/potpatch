...
from potpatch.utils import read_fortran_binary_block, read_fortran_binary_block_varioustype, write_fortran_binary_block
from potpatch.constant import *
...
class Lattice():
    ...
    def __init__(self, AL:np.ndarray, unit:str=None, fromwhere:str=["unkown"]) -> None:
    ...
    def modify_unit(self, target_unit:str):
        ...
        self.AL = self.AL / self.au_transformer[self.unit] * self.au_transformer[target_unit]
    ...
    def in_unit(self, target_unit:str):
        ...
        """
        self.unit_check(target_unit)
        return self.AL / self.au_transformer[self.unit] * self.au_transformer[target_unit]
...
class VR():
    ...
    def __init__(self, filename=None,  \
        ...
        if mesh     is not None: self.mesh: np.ndarray = mesh

    def __setattr__(self, name: str, value) -> None:
        if (name == "lattice") and (value is not None):
            assert type(value) is Lattice, f"type of self.lattice is not Lattice"
    ...
    def read_vr(self, filename, vr_fmt="PWmat"):
        ...
        self.lattice = Lattice(AL, self.fmt2unit[vr_fmt], fromwhere=self._latticeflag)
    ...
    def write_vr(self, filename: str, vr_fmt="PWmat", nnodes: int|None=None):
        """
        ...
        io = open(filename, "bw")
        # meta data
        write_fortran_binary_block(io, np.array(self.mesh.shape, dtype=np.int32), np.int32(nnodes))
        # AL data
...
class AtomConfig():
    ...
    def read_atoms(self, filename:str, atoms_fmt:str="PWmat") -> np.ndarray:
        self.filename = os.path.abspath(filename)
        io = open(filename, "r")
        ...
        self._latticeflag = f"({self.comment}) read fromfile({self.filename})"
        self.lattice = Lattice(AL, self.fmt2unit[atoms_fmt], fromwhere=self._latticeflag)
    ...
    def write_atoms(self, filename:str, atoms_fmt:str="PWmat"):
        ...
        if atoms_fmt == "PWmat": io.write(" Lattice vector (Angstrom)\n")
        ...
        for iislda in range(self.islda):
            for ikpt in range(self.nkpt):
                ...
                iislda_, ikpt_, self.weighkpt[ikpt], ak[0], ak[1], ak[2] = \
                    read_fortran_binary_block_varioustype(io, np.int32, np.int32, np.float64, np.float64, np.float64, np.float64)
