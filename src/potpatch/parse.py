...
def cli_arg_parse():
    ...
    parser_potpatch.add_argument("-i", "--input"
                        , help='specify the input file name. default is "potpatch.input"'
                        , default="potpatch.input")
    parser_potpatch.add_argument("-I", "--only-inspect"
                        , help='just inspecting ingredients without performing calculations.'
                        , action="store_true"
                        , dest="onlyinspect"
                        , default=False)
    parser_potpatch.add_argument("-V", "--version"
                        , action="store_true")
    
    subparsers = parser_potpatch.add_subparsers(help='sub-command help', dest="func")

    parser_mksupcl = subparsers.add_parser("mksupcl", help="a script to make supercell")
    parser_mksupcl.add_argument("-i", "--input"
                        , help='specify the input file name. default is "potpatch.input"'
                        , default="potpatch.input")
    ...
    if args.func is None:
            ...
            if args.version:
                ...
                ret_nt = NameTuple(inputfile=inputfile, onlyinspect=onlyinspect)
    else:
        if args.func == "mksupcl":
            PROG = "mksupcl"
            inputfile = args.input
